                          ...
                          double time_limit_ms)
{
    ...
    // Save CSV
    ofstream fout("ils_x_sweep.csv");
    fout << "X,avg,min,max\n";
    for (auto &st : all_stats)
        fout << st.X << "," << st.avg_score << "," << st.min_score << "," << st.max_score << "\n";
    fout.close();

    cout << "\nExperiment complete. Results written to ils_x_sweep.csv\n";
}


// ------------------------------------ ASSIGNMENT 7: LNS ------------------------------------ //
vector<int> destroyLNS(const vector<int>& tour, const vector<vector<int>>& d, const vector<Node>& nodes) {
    int current_k = tour.size();
    int remove_count = (int)(0.30 * current_k); // Remove 30%
    ...
    vector<pair<double, int>> node_scores; // {noisy_cost, index_in_tour}
    node_scores.reserve(current_k);

    for (int i = 0; i < current_k; ++i) {
        int prev_node = tour[(i + current_k - 1) % current_k];
        int curr_node = tour[i];
        int next_node = tour[(i + 1) % current_k];

        int valid_cost = d[prev_node][curr_node] + d[curr_node][next_node] + nodes[curr_node].cost;

        double noisy_cost = (double)valid_cost * noise_dist(rng); // randomization

        node_scores.push_back({noisy_cost, i});
    }

    sort(node_scores.rbegin(), node_scores.rend());

    // 3. Identify indices to remove
    unordered_set<int> indices_to_remove;
    for (int i = 0; i < remove_count; ++i) {
        indices_to_remove.insert(node_scores[i].second);
    }

    // 4. Construct the destroyed tour
    vector<int> destroyed;
    destroyed.reserve(current_k - remove_count);
    for (int i = 0; i < current_k; ++i) {
        if (indices_to_remove.find(i) == indices_to_remove.end()) {
            destroyed.push_back(tour[i]);
        }
    }
...
vector<int> repairLNS(const vector<int>& broken_tour, const vector<vector<int>>& d, const vector<Node>& nodes) {
    vector<int> tour = broken_tour;
    int n = d.size();
    int k = (n + 1) / 2; // Target size

    // 1. Mark currently used nodes
    ...
    // 2. Loop until tour is rebuilt to size k
    while ((int)tour.size() < k) {
        
        int best_node_to_insert = -1;
        int best_insertion_index = -1;
        double max_weighted_regret = -1e18;

        int current_tour_size = tour.size();

        // Evaluate every unused node
        for (int candidate = 0; candidate < n; ++candidate) {
            if (used[candidate]) continue;

            // Find best and second best insertion positions for this candidate
            int best_val = INT_MAX;
            int best_pos = -1;
            int second_best_val = INT_MAX;
            
            // Try inserting candidate between tour[i] and tour[i+1]
            for (int i = 0; i < current_tour_size; ++i) {
                int u = tour[i];
                int v = tour[(i + 1) % current_tour_size];

                // Cost change: add (u, cand), add (cand, v), remove (u, v), add cost(cand)
                int insertion_cost = d[u][candidate] + d[candidate][v] - d[u][v] + nodes[candidate].cost;

                if (insertion_cost < best_val) {
                    second_best_val = best_val;
                    best_val = insertion_cost;
                    best_pos = i;
                } else if (insertion_cost < second_best_val) {
                    second_best_val = insertion_cost;
                }
            }

            if (second_best_val == INT_MAX) second_best_val = best_val;

            // Calculate Regret Score
            // We want to maximize: w1 * (regret) - w2 * (cost)
            // Regret = 2nd_best - 1st_best
            double regret = (double)(second_best_val - best_val);
            double score = w1 * regret - w2 * (double)best_val;

            if (score > max_weighted_regret) {
                max_weighted_regret = score;
                best_node_to_insert = candidate;
                best_insertion_index = best_pos;
            }
        }

        // 3. Insert the winner
        if (best_node_to_insert != -1) {
            // Insert after index at position i + 1
            tour.insert(tour.begin() + best_insertion_index + 1, best_node_to_insert);
            used[best_node_to_insert] = true;
        } else {
...
vector<int> lns(const vector<vector<int>>& d, const vector<Node>& nodes, double time_limit_ms, bool useLS = false) {
    int n = d.size();
    int k = (n + 1) / 2;
    ...
    int loop_runs = 1;  // we already ran 1 LS
    ...
    while (true) {
        ...
        // --- Destroy ---
        vector<int> destroyed = destroyLNS(best, d, nodes);

        // --- Repair ---
        vector<int> repaired = repairLNS(destroyed, d, nodes);
        ...
        if (useLS)
        {
            improved = local_search_steepest(repaired, d, nodes, false);
        } else {
            improved = repaired;
        }
    ...
    cout << "\nNumber of iterations of main loop (LNS): " << loop_runs << endl;

    return best;
}

void run_lns_experiment(const vector<vector<int>> &d, const vector<Node> &nodes, double time_limit_ms, bool useLS = false) {
    const int N_EXPERIMENTS = 20;
    ...
    for (int run = 0; run < N_EXPERIMENTS; run++)
    {
        ...
        vector<int> best = lns(d, nodes, time_limit_ms, useLS);
        int best_obj = tour_objective(best, d, nodes);

        // record results
        lls_results.push_back(best_obj);
    ...
    if (useLS)
    {
        print_stats("LNS with LS after Destroy-Repair", lls_results, total_time_ms);
        export_tour_svg("LNS_with_LS_tour.svg", best_overall, nodes);
        export_tour_txt("LNS_with_LS_tour.txt", best_overall);
    } else {
        print_stats("LNS without LS after Destroy-Repair", lls_results, total_time_ms);
        export_tour_svg("LNS_without_LS_tour.svg", best_overall, nodes);
        export_tour_txt("LNS_without_LS_tour.txt", best_overall);
    }
}
...
int main(int argc, char **argv)
{
    ...
    cout << "\n===== Running LNS =====\n";
    run_lns_experiment(d, nodes, avg_msls_time_ms, false);

    run_lns_experiment(d, nodes, avg_msls_time_ms, true);
}